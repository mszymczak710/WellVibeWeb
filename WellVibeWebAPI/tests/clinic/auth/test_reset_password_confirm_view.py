...
@pytest.mark.parametrize(
    ...
    (
        ("wrong-uid", "wrong-token", "Invalid UID or user not found."),
        ("invalid_format", "some-token", "Invalid UID or user not found."),
    ),
)
...
def test_reset_password_confirm_inactive_user(
    ...
    expected_error = "User is inactive. Contact support for assistance."
    expected_response = {"error": [expected_error]}
    assert (response.status_code, response.data) == (
        status.HTTP_400_BAD_REQUEST,
        expected_response,
...
def test_reset_password_confirm_nonexistent_user(
    ...
    expected_error = "Invalid UID or user not found."
    expected_response = {"error": expected_error}
...
def test_reset_password_confirm_token_issues(
    ...
    expected_error = "The reset password link is invalid or has expired. Please request a new one."
    expected_response = {"error": [expected_error]}
...
@pytest.mark.django_db
def test_password_reset_confirm_rate_limiting(
    api_client, reset_password_url, set_password_data
):
    for _ in range(50):
        response = api_client.post(
            reset_password_url, data=set_password_data, format="json"
