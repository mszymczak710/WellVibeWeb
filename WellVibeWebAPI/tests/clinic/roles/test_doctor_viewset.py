...
def test_filter_doctor_by_multiple_specializations(
    ...
    url = reverse("doctor-list")
    ordering = "readable_id"
    specialization_query = ",".join(filter_values)
    response = api_client.get(
        f"{url}?specializations__name={specialization_query}&ordering={ordering}"
    )
...
def test_unsupported_http_methods_are_rejected_for_doctor_detail(
    request, doctor_instances, http_method, user_fixture
):
    api_client, _ = request.getfixturevalue(user_fixture)
    doctor = doctor_instances[0]
    url = reverse("doctor-detail", args=(doctor.pk,))
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            status.HTTP_400_BAD_REQUEST,
            {
                "specializations": ["This list may not be empty."],
            },
        ),
        (
...
def test_doctor_cannot_set_nonexistent_specialization(
    ...
    expected_error = f'Invalid pk "{non_existent_specialization_id}" - object does not exist.'
...
@pytest.mark.django_db
def test_doctor_cannot_edit_own_data_rate_limiting(
    authenticated_doctor, doctor_patch_data
):
    api_client, doctor = authenticated_doctor
    url = reverse("doctor-detail", args=(doctor.pk,))
